...
namespace CGE
{
	...
	protected:
		static ParamType paramTextureVertexName;
	...
	{
	public:
		void forwardDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius, float intensity);//see in 'LiquidationProcessor'
		virtual void pushLeftDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius, float intensity, float angle = 3.14159f / 2.0f);//see in 'LiquidationProcessor'
	};

...
namespace CGE
{
	//Attention: the CGECurveTexFilter will use the "TEXTURE_START"
	//Any textures add-in this one, you should use " TEXTURE_START + ? " instead.
	...
	public:
		...
		inline const float* getCurveRGB(size_t* len = NULL) const { if(len != NULL) *len = m_curve.size(); return &m_curve[0][0]; };
	...
	//Deprecated, use "CGECurveFilter" instead.

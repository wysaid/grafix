...
namespace CGE
{
	...
	{
	public:
		AnimationWithChildrenInterface(float start, float end) : AnimationInterfaceAbstract<ActionType>(start, end) {}
		virtual ~AnimationWithChildrenInterface()
		{
			clearChildren(true);
		}

		std::vector<AnimationWithChildrenInterface*>& children() { return m_childAnimation; }

		virtual void clearChildren(bool bDelete = true)
		{
			if(bDelete)
			{
				for(typename std::vector<AnimationWithChildrenInterface*>::iterator iter = m_childAnimation.begin(); iter != m_childAnimation.end(); ++iter)
				{
					delete *iter;
				}
			}
		...
		virtual void pushChild(AnimationWithChildrenInterface* child) { assert(child != this && child != NULL); m_childAnimation.push_back(child); }

		virtual void _renderWithFather(AnimationWithChildrenInterface*) = 0; // 必须实现(若有特殊用法请自行写成空函数).

		virtual bool run(float currentTime)
		{
			AnimationInterfaceAbstract<ActionType>::run(currentTime);
			for(typename std::vector<AnimationWithChildrenInterface*>::iterator iter = m_children2Run.begin(); iter != m_children2Run.end();)
			{
				AnimationWithChildrenInterface* anim = *iter;
		...
		{
			AnimationInterfaceAbstract<ActionType>::animationStart();
			for(typename std::vector<AnimationWithChildrenInterface*>::iterator iter = m_childAnimation.begin(); iter != m_childAnimation.end(); ++iter)
			{
				(*iter)->animationStart();
			}
		...
		{
			AnimationInterfaceAbstract<ActionType>::animationEnd();
			for(typename std::vector<AnimationWithChildrenInterface*>::iterator iter = m_children2Run.begin(); iter != m_children2Run.end(); ++iter)
			{
				(*iter)->animationEnd();
			}
			m_children2Run.clear();
	...
	protected:
		std::vector<AnimationWithChildrenInterface*> m_childAnimation;
		std::vector<AnimationWithChildrenInterface*> m_children2Run;

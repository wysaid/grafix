...
namespace CGE
{
	...
	public:
		...
		CGE_LOG_CODE(
		static bool sRemoveMe(std::vector<TimeActionInterfaceAbstract*>& vec, TimeActionInterfaceAbstract* action);
		//保存所有存在的action, 探测内存泄漏或者进行一些全局设置。
		static std::vector<TimeActionInterfaceAbstract*> s_actionManager; 
		)
